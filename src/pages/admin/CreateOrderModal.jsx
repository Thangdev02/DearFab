...
function CreateOrderModal({ show, onHide, onSave }) {
  ...
  const handleAddItem = () => {
    setOrder((prev) => ({
      ...
      orderedItems: [
        ...prev.orderedItems,
        {
          id: (prev.orderedItems.length + 1).toString(),
          name: '',
          price: 0,
          quantity: 1,
          size: 'M',
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      const { success, order: savedOrder, message } = await saveOrder(order);
      if (success) {
        onSave(savedOrder);
        onHide();
      } else {
    ...
    <Modal show={show} onHide={onHide}>
      ...
      <Modal.Body>
        <Form>
          ...
          <Button variant="secondary" onClick={handleAddItem}>Thêm sản phẩm</Button>
        </Form>
      </Modal.Body>
      <Modal.Footer>
